...
System.Clear()
System.Title("Bandera777 - By Kliter")
lllll = ['proxies.txt', 'webhooks.txt', 'tokens.txt']
if not os.path.exists('./Bandera777'):
    os.system('md Bandera777')
    with open('./Bandera777/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
elif os.path.exists('./Bandera777') and not os.path.exists('./Bandera777/useragents.txt'):
    with open('./Bandera777/useragents.txt', 'w') as file:
        ua = UserAgent()
        file.write('\n'.join([ua.random for _ in range(50)]))
else:
...
| $$  \ $$ /$$__  $$| $$  | $$| $$  | $$| $$_____/| $$       /$$__  $$  /$$/      /$$/      /$$/    
| $$$$$$$/|  $$$$$$$| $$  | $$|  $$$$$$$|  $$$$$$$| $$      |  $$$$$$$ /$$/      /$$/      /$$/     
|_______/  \_______/|__/  |__/ \_______/ \_______/|__/       \_______/|__/      |__/      |__/      
...
def print_logo():
 ...
 |____/ \__,_|_| |_|\__,_|\___|_|  \__,_/_/    /_/    /_/    
    ...
    > github.com/Bandera777'''
    b = Colorate.Vertical(Colors.DynamicMIX((Col.red, Col.gray)), Center.XCenter(banner))
...
def random_useragent():
    with open('./Bandera777/useragents.txt', 'r') as file:
    ...
    if ch == '23':
        ...
        if len(tokens()) > 0:
            ...
            for token in tokens(): 
                ...
                yon = input_()
                if yon.lower().startswith('y'):
                    with open("./Bandera777/valids.txt", 'w') as file:
                        file.write('\n'.join(valids))
                    with open("./Bandera777/invalids.txt", "w") as file:
                        file.write('\n'.join(invalids))
                    print_("[!!] Saved In /Bandera777, Press Enter to continue")
                    input()
                await tokenTools()
    if ch == '24':
        print_logo()
